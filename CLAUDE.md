...
In production it runs as a systemd service (`deploy/medisite.service`) under a dedicated unprivileged `medisite` system user, with `WorkingDirectory=/opt/medisite` — the same directory holding the binary, `templates/`, `static/`, and `content/`, since those are read from disk relative to the working directory (see below). `sudo ./deploy/install.sh` builds, installs/updates all of the above, and restarts the service — it's safe to re-run for every redeploy. The one thing it deliberately never overwrites is `content/` once it already exists on the server (only seeded there on the very first install), since posts and portfolio entries are meant to be edited live on the server, not shipped by the deploy script. `sudo ./deploy/update.sh` is the opt-in, content-only counterpart for when the repo *is* the source of truth for content: it replaces **only** content/ (staged first, then swapped in by rename, keeping the previous copy as `content.prev`), restarts the service, and supports `--rollback` to swap `content.prev` back. It assumes install.sh has already run; the binary, templates/, static/, config, unit file, and service user are install.sh's job alone, so the two scripts don't overlap.

There are deliberately **two** config files, not one: the repo-root `config.yaml` is local-dev-only (picked up automatically by `go run .`, relative paths, no elevated permissions needed) and is never installed anywhere; `deploy/config.production.yaml` is what `install.sh` actually installs to `/etc/medisite/config.yaml` (the FHS-conventional place for config, separate from the `/opt` app/data bundle) and points `log_file` at the absolute path `/var/log/medisite/medisite.log`. They started as one shared file, but pointing the single `log_file` at an absolute, root-owned production path broke plain local `go run .` (it tried to create `/var/log/medisite` and failed without root) — splitting them keeps local dev needing zero setup while production still gets FHS-conventional paths. Don't recombine them without preserving that property.
